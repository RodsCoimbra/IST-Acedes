...
__global__ void SAD_GPU(int *d_CurrentBlock, int *d_SearchArea, int rowIdx, int colIdx, int centerX, int centerY, int distance, Parameters p, int *d_results)
{
    ...
    int i = tid << 2;
    int posX = centerX + (blockIdx.y - 3) * distance + SEARCH_RANGE;
    int posY = centerY + (blockIdx.z - 3) * distance + SEARCH_RANGE;
        ...
        (0 <= (colIdx + posY)) && ((colIdx + posY) < p.width))
    {
        ...
        if (tid == 0)
        {
            d_results[blockIdx.z + 7 * blockIdx.y] = sad_shared[0];
        }
    }
    else
    {
        d_results[blockIdx.z + 7 * blockIdx.y] = BigSAD;
    }
}
...
void StepSearch(BestResult *bestResult, int *d_CurrentBlock, int *d_SearchArea, int rowIdx, int colIdx, Parameters p, int *d_results, int *results)
{
    ...
    int Distance = (p.searchRange) >> 2; // Initial distance = search range/2
    while (Distance >= 1)
    {
        SAD_GPU<<<gridSize, blockSize>>>(d_CurrentBlock, d_SearchArea, rowIdx, colIdx, CenterX, CenterY, Distance, p, d_results);

        if (cudaMemcpy(results, d_results, 49 * sizeof(int), cudaMemcpyDeviceToHost) != cudaSuccess)
        {
	...
	d7 = 3*Distance;
        ...
        { // printf("results[%d] = %d\n",j,results[j]);
            if (results[j] < bestResult->sad)
            {
                ...
                bestResult->vec_x = (j / 7) * Distance + CenterX - d7;
                bestResult->vec_y = (j % 7) * Distance + CenterY - d7;
            }
        }
        // printf("best is %d\n",bestResult->sad);
...
int main(int argc, char **argv)
{
    ...
    if (cudaMallocHost((int **)&results, 49 * sizeof(int)) != cudaSuccess)
    {
    ...
    if (cudaMalloc((void **)&d_results, 49 * sizeof(int)) != cudaSuccess)
    {
    ...
    for (int frameNum = 0; frameNum < p.frames; frameNum++)
    {
        ...
        MotionEstimation(motionVectors, curr_frame, d_curr_frame, ref_frame, d_ref_frame, p, results, d_results, d_CurrentBlock, d_SearchArea);
