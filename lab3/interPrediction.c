        ...
        (0 <= (colIdx + posY)) && ((colIdx + posY) < p.width))
    {
        ...
        for (int i = 0; i < p.blockSize; i++)
            for (int j = 0; j < p.blockSize; j++)
                sad += abs(CurrentBlock[i][j] - SearchArea[posX + i][posY + j]);
...
void MotionEstimation(BestResult **motionVectors, int **curr_frame, int **ref_frame, Parameters p)
{
    ...
    int **CurrentBlock = (int **)malloc(p.blockSize * sizeof(int *));
    for (int i = 0; i < p.blockSize; i++)
        CurrentBlock[i] = (int *)malloc(p.blockSize * sizeof(int));

    int **SearchArea = (int **)malloc((2 * p.searchRange + p.blockSize) * sizeof(int *));
    for (int i = 0; i < (2 * p.searchRange + p.blockSize); i++)
        SearchArea[i] = (int *)malloc((2 * p.searchRange + p.blockSize) * sizeof(int));
