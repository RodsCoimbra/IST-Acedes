...
void SAD(BestResult *bestResult, int *d_CurrentBlock, int *d_SearchArea, int rowIdx, int colIdx, int k, int m, Parameters p)
{
        ...
        (0 <= (colIdx + posY)) && ((colIdx + posY) < p.width))
    {
        ...
        int tamanhoy = BLOCK_SIZE;
        dim3 gridDist(1, p.blockSize, 1); // X -> p.blockSize (threadIdx.x)
        dim3 blockDist(tamanhoy, 1, 1);   // Y    -> tamanhoy (blockIdx.y)

        int results[tamanhoy] = {0}, *d_results;
        if (cudaMalloc((void **)&d_results, tamanhoy * sizeof(int)) != cudaSuccess)
        {
            printf("CANNOT ALLOCATE d_results");
            exit(0);
        }
        if (cudaMemcpy(d_results, results, tamanhoy * sizeof(int), cudaMemcpyHostToDevice) != cudaSuccess)
        {
            printf("FAILED TO COPY results DATA TO THE DEVICE\n");
            exit(0);
        }
        sum_SAD<<<gridDist, blockDist>>>(d_CurrentBlock, d_SearchArea, d_results, posX, posY, (2 * p.searchRange + p.blockSize));

        if (cudaMemcpy(results, d_results, tamanhoy * sizeof(int), cudaMemcpyDeviceToHost) != cudaSuccess)
        {
...
void fullSearch(BestResult *bestResult, int *CurrentBlock, int *SearchArea, int rowIdx, int colIdx, Parameters p)
{
    ...
    for (int iStartX = -p.searchRange; iStartX < p.searchRange; iStartX++)
    {
        ...
        {
            SAD(bestResult, d_CurrentBlock, d_SearchArea, rowIdx, colIdx, iStartX, iStartY, p);
        }
    }
