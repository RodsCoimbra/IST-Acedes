...
/************************************************************************************/
__global__ void getBlock(int *block, int *frame, int i, int j, Parameters p)
{
...
void fullSearch(BestResult *bestResult, int *d_CurrentBlock, int *SearchArea, int rowIdx, int colIdx, Parameters p, int *d_results)
{
...
/************************************************************************************/
void MotionEstimation(BestResult **motionVectors, int *d_curr_frame, int *ref_frame, Parameters p)
{
    ...
    for (int rowIdx = 0; rowIdx < (p.height - p.blockSize + 1); rowIdx += p.blockSize)
    {
        for (int colIdx = 0; colIdx < (p.width - p.blockSize + 1); colIdx += p.blockSize)
        {
            ...
            // printf("Entrei, rowIdx = %d, colIdx = %d\n", rowIdx, colIdx);
            getBlock<<<gridBlock, blockBlock>>>(d_CurrentBlock, d_curr_frame, rowIdx, colIdx, p);
            ...
            {
            case FSBM:
                fullSearch(bestResult, d_CurrentBlock, SearchArea, rowIdx, colIdx, p, d_results);
                break;
            default:
                break;
            }
...
int main(int argc, char **argv)
{
    ...
    for (int frameNum = 0; frameNum < p.frames; frameNum++)
    {
        ...
        MotionEstimation(motionVectors, d_curr_frame, ref_frame, p);
