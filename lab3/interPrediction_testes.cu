...
{
    {
        ...
        int f = i / BLOCK_SIZE;
        int g = i % BLOCK_SIZE;
        __shared__ int sad_shared[1024];

        // computes SAD disparity, by comparing the current block with the reference block at (k,m)
        sad_shared[i] = abs(d_CurrentBlock[i] - d_SearchArea[(posX + f) * step_search + (posY + g)]);
        __syncthreads();

        for (int j = blockDim.z >> 1; j > 0; j = j >> 1)
        {
            if (i < j)
            {
                sad_shared[i] += sad_shared[i + j];
            }
            __syncthreads();
        }
        // compares the obtained sad with the best so far for that block
        if (i == 0)
        {
            d_results[posY * 2 * SEARCH_RANGE + posX] = 5;
        }
    }
}
...
void fullSearch_GPU(BestResult *bestResult, int *d_CurrentBlock, int *d_SearchArea, int rowIdx, int colIdx, Parameters p, int *d_results, int *results)
{
    ...
    bestResult->vec_x = 0;
    bestResult->vec_y = 0;
    dim3 gridSize(2 * SEARCH_RANGE, 2 * SEARCH_RANGE, 1);
    dim3 blockSize(1, 1, BLOCK_SIZE * BLOCK_SIZE);
